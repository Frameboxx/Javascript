var xJs = (function(obj){
	...
	// Capitalize Function
	obj.capitalize = function(w){
		if(!w && w!==''){
			// use the chained invocation
			this.sel.value.replace(/\b\w/g, function(txt){
				return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
			});
		} else {
			// use w and return the capitalized text
			return w.replace(/\b\w/g, function(txt){
				return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
			});
		}
	}
